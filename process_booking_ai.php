...
try {
    ...
    if (!$user_id) {
        ...
        if ($existing_user) {
            ...
            $stmt_new = $pdo->prepare("INSERT INTO users (name, phone, password, role) VALUES (?, ?, ?, 'client')");
    ...
    $stmt = $pdo->prepare("INSERT INTO reservations (user_id, customer_name, customer_phone, event_date, event_location, total_price, duration_days, distance_km, discount_amount, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')");
    $stmt->execute([$user_id, $customer_name, $customer_phone, $event_date, $event_location, $pricing['total'], $duration, $distance, $pricing['discount']]);
    ...
    die("Erreur de réservation : " . $e->getMessage());
}
?>
